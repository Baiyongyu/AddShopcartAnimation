...
{
    UIImageView *_cartAnimView;     // 图片旋转动画
    ChoseView *choseView;
    UIView *bgview;
    CGPoint center;
...
#pragma mark - 此处为加入购物车之后的旋转飞入购物车动画
...
#pragma mark - 此处为加入购物车之后的旋转飞入购物车动画
#pragma mark - 这是从网上随便找的demo，只是添加了动画效果
#warning 动画效果，只有这些代码，喜欢的小伙伴，可以做参考
- (void)sure {
    // 旋转跳跃小动画走起
    _cartAnimView = [[UIImageView alloc] initWithFrame:CGRectMake(10, 180, 90, 90)];
//    [_cartAnimView sd_setImageWithURL:[NSURL URLWithString:self.popImgArray[0]] placeholderImage:[UIImage imageNamed:@"placeholderMid"]];
    _cartAnimView.image = [UIImage imageNamed:@"1.jpg"];
    [choseView addSubview:_cartAnimView];
    ...
    [UIView animateWithDuration:1.0 animations:^{
        _cartAnimView.frame = CGRectMake(self.view.frame.size.width - 55, - (self.view.frame.size.height - CGRectGetHeight(self.view.frame) - 40), 0, 0);
    } completion:^(BOOL finished) {
        // 动画完成后弹框消失
        [self dismiss]; // 点击半透明部分或者取消按钮，弹出视图消失
    }];
}
